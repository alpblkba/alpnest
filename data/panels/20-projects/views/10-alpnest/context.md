...
The next implementation target is the local git tracker. It should be implemented as a safe read-only generator script first, then wired into the projects panel.

Do not start calendar write access until:

- git tracker exists
- theme/readability pass is usable
- notification deduplication exists
- calendar read-only sync has been tested
